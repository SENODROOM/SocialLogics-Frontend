...
export default function SearchBar({
  ...
  const handleSubmit = async (q = query, p = platform, ct = contentType) => {
    ...
    try {
      ...
      if (onSearch) {
        ...
        navigate(`/search?q=${encodeURIComponent(q)}&platform=${p}&ct=${ct}`);
      }
    } finally {
      setLoading(false);
    }
  ...
  const fs = large
    ? {
        input: "1.05rem",
        pad: "18px 20px 18px 52px",
        btnPad: "18px 36px",
        btnFs: "0.8rem",
      }
    : {
        input: "0.9rem",
        pad: "13px 16px 13px 44px",
        btnPad: "13px 22px",
        btnFs: "0.72rem",
      };
      ...
      <div style={{ display: "flex", gap: 10, position: "relative" }}>
        ...
        <div style={{ position: "relative", flexShrink: 0 }}>
          <select
            ...
            style={{
              ...
              cursor: "pointer",
              borderColor:
                activePlatform?.id !== "all"
                  ? activePlatform?.color + "60"
                  : undefined,
            }}
          >
            {PLATFORMS.map((p) => (
              <option key={p.id} value={p.id}>
                {p.id === "all" ? "⬡ All Platforms" : `${p.icon} ${p.name}`}
              </option>
            ))}
          </select>
          <span
          ...
          {VOICES_SUPPORTED && (
            <button
              ...
              style={{
                ...
                borderRadius: 4,
                padding: "4px 8px",
                color: listening ? "#ff3366" : "var(--c-text3)",
                fontSize: "0.9rem",
        ...
        {showFilters && (
          <button
            ...
            style={{
              padding: large ? "18px 14px" : "13px 12px",
              flexShrink: 0,
              ...
              color: showAdvanced ? "var(--c-cyan)" : "var(--c-text3)",
              fontSize: "1rem",
            }}
          >
            ⚙
            ...
            {CONTENT_TYPES.map((ct) => (
              <button
                ...
                style={{
                  ...
                  color:
                    contentType === ct.id ? "var(--c-cyan)" : "var(--c-text3)",
                  fontSize: "0.72rem",
          ...
          {suggestions.map((s, i) => (
            <div
              ...
              style={{
                ...
                cursor: "pointer",
                background:
                  i === suggIdx ? "rgba(0,212,255,0.08)" : "transparent",
                display: "flex",
                alignItems: "center",
                gap: 12,
                borderBottom:
            ...
            >
              ...
              {s.type === "history" && (
                <span style={{ fontSize: "0.6rem", color: "var(--c-text3)" }}>
                  history
                </span>
              )}
            </div>
